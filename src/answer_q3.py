...
def answer_q3(spark, store_id=None):
    ...
    try:
        ...
        df= df.select(['button_product_id', 'user_author_id', 'date'])
        df2 = df2.select(['product_id', 'name'])
        df = df.withColumnRenamed('name', 'product_name')
        df = df.join(df2, df['button_product_id'] == df2['product_id'], how='inner')
        ...
        df = df.groupBy(['minute', 'name']).agg(F.countDistinct('user_author_id').alias('unique_users'))
        df = df.sort('minute')
...
# create table q3 (minute timestamp, product_name varchar, unique_users float);
